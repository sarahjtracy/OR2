...
class player(object):
  def __init__(self, name, tp, mp, ast, teamAst, teamFg, fg, 
               ...
               pts, trb, sa, df, lbr, cd, cs, cost, pos):
    self.name = name
    self.tp = tp # 3P
    self.mp = mp
    self.ast = ast
    ...
    self.cost = cost         # cost of player
    self.pos = pos           # position of player
    self.UPer = self.setUPer()
    self.intangibles = self.setIntangiblesScore()
